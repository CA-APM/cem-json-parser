...
public class CemJavaPlugin implements CemPluginApiV2 {
    public ConcurrentHashMap<Integer,String> chunkedBody = new ConcurrentHashMap<Integer,String>();
    ...
    /**
     * Check if requestHeader "Content-Type" contains "json".
     */
    public CemPluginOutput requestHeader(int id, String name, String value) {
        if (name.equals("Content-Type")) {
            if (value.indexOf("json") > -1) {
                chunkedBody.put(new Integer(id),"");
            }
        }
        return null;
    ...
    public CemPluginOutput requestBody(int id, byte[] str) {
        try {
            ...
            if (chunkedBody.containsKey(key)) {
                String bodyChunk = new String(str);
                bodyChunk = chunkedBody.get(key) + bodyChunk;
                chunkedBody.put(key,bodyChunk);
            }
        } catch (Exception e) {
            // ignore
        }
    ...
    public CemPluginOutput endRequest(int id) {
        try {
            ...
            if (!chunkedBody.containsKey(requestKey)) {
                return null;
            }

            String body = chunkedBody.get(requestKey);
            chunkedBody.remove(requestKey);
